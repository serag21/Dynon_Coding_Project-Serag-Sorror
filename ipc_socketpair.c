...
int main() {
    ...
    //was facing partial write issues where not all bytes of struct were written to socket,
    //could be due to various reasons such as buffer size or interruptions in communication.
    //SOLUTION: Added check after each 'write' operation to verify exact number of bytes from each array index in 'dataToSend' was successfully written and add a retry
